...
def model_config(model: WishartProcessModel, kernel: dict) -> dict:
    ...
    0.2, "variance": 1.0}`` recording how the basis spectral density was built
    (the closure itself cannot be serialised).
    """
    return {
        "kernel": kernel,
        "basis": {
            "max_freq": model.basis.max_freq,
            "num_dims": model.basis.num_dims,
            "tol": model.basis.tol,
        },
        "model": {
...
def build_model(config: dict) -> WishartProcessModel:
    ...
    b = config["basis"]
    basis = TruncatedFourierBasis(b["max_freq"], b["num_dims"], density, tol=b["tol"])
